...
async function resolveOne(input) {
  ...
  try {
    ...
    const itemIds = [...new Set([...html.matchAll(/(?:item_?id|itemid)["'\s:=\\]+(\d{5,})/ig)].map(m => m[1]))].slice(0, 20);
    const shopIds = [...new Set([...html.matchAll(/(?:shop_?id|shopid)["'\s:=\\]+(\d{5,})/ig)].map(m => m[1]))].slice(0, 20);
    return {
      input,
      url,
      title,
      navigations: [...new Set(seen)],
      itemIds,
      shopIds,
      bodyText: bodyText.replace(/\s+/g, ' ').slice(0, 1500)
    };
  } catch (e) {
