const CACHE_VERSION = 'achou-levou-v84-cupom-manual';
...
self.addEventListener('install', () => {
    console.log('Achou Levou interface v84 instalada.');
    self.skipWaiting();
});

self.addEventListener('activate', (event) => {
    console.log('Achou Levou interface v84 ativada. Limpando caches antigos.');
    event.waitUntil(
        caches.keys()
            ...
            .then(clients => Promise.all(clients.map(client => {
                client.postMessage({ type: 'ACHOU_LEVOU_UPDATED', version: '84' });
