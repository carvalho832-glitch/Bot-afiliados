...
# O painel recebe a Fase 22 automaticamente no boot e, por isso, pode ficar
# modificado mesmo sem edição manual. Se panel.html for a ÚNICA alteração
# rastreada, restauramos a cópia versionada antes de atualizar. Qualquer outra
# alteração continua bloqueando a atualização.
TRACKED_CHANGES="$(
  {
...
if [[ -n "$TRACKED_CHANGES" ]]; then
  NON_GENERATED_CHANGES="$(printf '%s\n' "$TRACKED_CHANGES" | grep -v '^whatsapp-bot/panel\.html
...
exit 2
 || true)"
  ...
  if [[ -z "$NON_GENERATED_CHANGES" ]]; then
    say "🧹 panel.html contém somente a injeção automática do painel; restaurando a versão do Git antes da atualização..."
