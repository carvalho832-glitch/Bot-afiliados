...
async function getInfo() {
  ...
  const nextRun = lastSend && intervalMinutes
    ? new Date(lastSend.getTime() + intervalMinutes * 60000)
    : null;

  const now = new Date();
  const remainingMinutes = nextRun
    ? Math.ceil((nextRun.getTime() - now.getTime()) / 60000)
    : null;
...
function render(info) {
  ...
  const fila = info.queueRunning === true ? 'ligada' : info.queueRunning === false ? 'parada' : 'não lida';
...
<body>
  <div class="card">
    ...
    <div class="big">
      ...
      <div class="status">${esc(info.statusText)}</div>
    </div>
    ...
    <div class="footer">
      ...
      A página atualiza sozinha a cada 30 segundos.<br><br>
