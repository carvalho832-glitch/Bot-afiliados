...
// --- CHAMADA DA IA (REVISADA PARA NÃO REPETIR) ---
async function pedirTextoParaIA(produto, preco) {
    // Adicionamos um número aleatório ao prompt para forçar a IA a mudar o texto
    const randomID = Math.floor(Math.random() * 1000);
    const promptTexto = `ID:${randomID}. Você é um vendedor de ofertas. Crie uma frase CURTA e ÚNICA para vender: ${produto} por ${preco}. Use emojis. Não use aspas.`;
    ...
    try {
        ...
        return "Confira essa oferta incrível que acabamos de encontrar! 🔥";
    } catch (e) {
        return "Preço imbatível por tempo limitado. Aproveite! 🚀";
    }
}

// --- PUXAR DADOS (SEUS SELETORES ORIGINAIS) ---
    ...
    try {
        ...
        if (urlMatch) {
            ...
            if (json.data) {
                ...
                displayPor.value = vPor; displayDe.value = vDe;
            }
        }
    } catch (e) { console.log(e); } finally { loader.style.display = 'none'; }
};

// --- GERAR MENSAGEM ---
btnGerar.onclick = async () => {
    if(!displayProduto.value || displayProduto.value === "Buscando...") return alert("Puxe os dados!");
...
// --- HISTÓRICO ---
btnSalvar.onclick = () => {
    if(!displayProduto.value) return;
...
function renderizarOfertas() {
    ...
    ofertasSet.forEach(o => {
        ...
        div.innerHTML = `<pre style="font-size:13px; color:#e6edf3; white-space:pre-wrap; margin:0 0 15px 0; font-family: monospace;">${o.texto}</pre>
...
window.apagar = (id) => { if(confirm("Apagar?")) { ofertasSet = ofertasSet.filter(o => o.id !== id); localStorage.setItem('ofertas_achou_levou', JSON.stringify(ofertasSet)); renderizarOfertas(); } };
