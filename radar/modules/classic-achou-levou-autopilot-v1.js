...
if (!box) { box = document.createElement('div'); box.id = STATUS_ID; box.style.cssText = 'position:fixed;left:12px;right:12px;bottom:178px;z-index:2147483647;padding:13px 16px;border-radius:15px;font:800 13px/1.45 system-ui;text-align:center;box-shadow:0 12px 36px #0009;backdrop-filter:blur(8px);'; document.documentElement.appendChild(box); }
...
if (!response.ok || !body?.ok) throw new Error(body?.error || body?.detalhe || `HTTP ${response.status}`); return body;
...
showStatus(`🔎 Puxando os dados do produto: ${clean(item.title).slice(0, 62)}...`); const params = new URLSearchParams({ url: item.affiliateUrl }); const data = await request(`${API_BASE}/shopee/produto?${params.toString()}`, {}, 130000);
...
showStatus(`☁️ Salvando e verificando a oferta ${run.successCount + 1}/${run.target}...`); const sourceId = `phase24-auto:${run.id}:${item.id}`;
const before = await request(`${SHARED_ENDPOINT}?t=${Date.now()}`, {}, 35000); const previous = Array.isArray(before.offers) ? before.offers : [];
const sameSource = previous.find(offer => clean(offer.sourceId) === sourceId);
const duplicate = previous.find(offer => clean(offer.sourceId) !== sourceId && clean(offer.link) === clean(item.affiliateUrl) && clean(offer.message || offer.mensagem || offer.texto) === clean(message));
if (duplicate) { const error = new Error('Esta oferta já existe na fila compartilhada e não será contada novamente.'); error.code = 'DUPLICATE_OFFER'; throw error; }
let offerId = clean(sameSource?.id);
if (!offerId) {
...
offerId = clean(saved.offer?.id);
}
if (!offerId) throw new Error('A fila compartilhada não devolveu o ID da oferta.');
...
const nonRetryable = error?.code === 'DUPLICATE_OFFER'; const attempts = nonRetryable ? MAX_OFFER_ATTEMPTS : Number(item.attempts?.offer || 0) + 1; const message = clean(error?.message || error);
if (!nonRetryable && attempts < MAX_OFFER_ATTEMPTS) {
...
const result = await patchItem(run, item, { stage: 'failed-offer', attempts: { ...(item.attempts || {}), offer: attempts }, lastError: message, reason: nonRetryable ? 'Oferta duplicada ignorada para preservar a meta de ofertas únicas.' : 'Produto ignorado após falhar ao puxar, gerar ou salvar a oferta.', failedAt: new Date().toISOString(), event: { type: 'failure', message: nonRetryable ? `Oferta duplicada ignorada: ${message}` : `Produto ignorado após ${attempts} tentativas: ${message}` } });
...
showStatus(`🤖 Produção automática: ${run.successCount}/${run.target} salvas • ${run.failureCount} falha(s).`); await patchRun(run, { status: 'running', stage: 'achou-levou', currentItemId: item.id }); await patchItem(run, item, { stage: 'product-pulling', reason: 'Puxando dados oficiais do produto no Achou Levou.' });
try {
const product = await pullProduct(item); fillScreen(item, product); await patchItem(run, item, { stage: 'message-generating', productData: product, reason: 'Dados do produto puxados. Gerando mensagem com IA.' });
const generated = await generateMessage(item, product); fillScreen(item, product, generated.message); await patchItem(run, item, { stage: 'saving-offer', message: generated.message, productData: product, reason: 'Mensagem gerada. Salvando e verificando na fila compartilhada.' });
...
try {
if (!await waitForPage()) throw new Error('A tela do Achou Levou não terminou de carregar.'); const run = await currentRun();
if (!run) { showStatus('Radar automático aguardando uma nova produção.', 'warning'); return; }
...
const item = pendingItem(run); if (!item) { await patchRun(run, { status: 'running', stage: 'link', currentItemId: '', event: { type: 'recovery', message: 'Nenhum item pendente no Achou Levou. Retornando ao garimpo.' } }); openShopee(); return; }
